...
export default function DriverCard({ driver }) {
  const { slug, firstName, lastName, nationality, number, status, teams } = driver

  const currentTeam = teams.find((t) => t.to === null) ?? teams[teams.length - 1]

  return (
    <Link href={`/drivers/${slug}`} className="group block">
      <div className="relative overflow-hidden bg-surface aspect-[3/4] rounded-sm">

        {/* Photo placeholder — remplacé par une vraie image plus tard */}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 via-black/50 to-transparent">
          <p className="text-[11px] text-text-muted mb-1">
            {flagEmoji[nationality] ?? '🏁'} {currentTeam?.name}
          </p>
